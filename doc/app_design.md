...
graph TD
    ...
    C --> D{OPENAI_API_KEY 設定済み?}
    D -- 未設定 --> E["警告表示"]
    D -- 設定済 --> F["sidebar_settings()"]
    F --> G{tool_mode}
    ...
    I --> P["ui_web_search()"]
    P --> Q["stream_web_search()"]
    Q --> R["OpenAI web_search ツール"]
    I --> S["ui_image_generation()"]
    S --> T["OpenAI Images API"]
    I --> U["ui_rag_management()"]
    U --> V["list_vector_stores()/files()"]
    V --> W["OpenAI Vector Store API"]
    H --> X["Responses API (Step1/Step2)"]
    H --> Y["ui_code_fix_followups()"]
    Y --> Z["Responses API 再呼出"]
```
